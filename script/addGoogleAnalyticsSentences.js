...
function processFamilyModel(model) {
	...
	while (idx.hasNext() == true) {
		...
		// find elements that are ps:pvscltext type
...
function modifyFile(path, fileName) {
	try {
		...
		// gather the new file content adding google analytics sentences
		var newFileContent = ''
		while (inLine != null) {
			var outLine = inLine.toString()
			var found = inLine.toString().match(/PVSCL:IFCOND\([a-zA-Z0-9_ ]*[\,\)]/);
			var firstSpaces = ''
			if (found) {
				for ( var i = 0; i < found.length; i++) {
					var featureName
					if(found[i].slice(-1) == ')') {
						// I think that only it can work with conditions that delete the whole row
						// featureName = found[i].replace("PVSCL:IFCOND(", "").replace(")", "")
						// inLine = inLine.replace(found[i], "PVSCL:IFCOND("+ featureName + ")_gaq.push(['"+ featureName + "', 'loadStart']")
					} else {
						// work over the conditions that includes the LINE flag
						// find the feature name
						featureName = found[i].replace("PVSCL:IFCOND(", "").replace(",", "").trim()
						// count the number of the first spaces to start the new line in the same column
						firstSpaces = inLine.match(/^\s*/)[0]
						// retrive the whole condition
						// var fullCondition = inLine.toString().match(/PVSCL:IFCOND\([a-zA-Z0-9_, ]*[\)]/)[0];
						// add the google analytics sentence
						outLine += "\n" + firstSpaces + "_gaq.push(['"+ featureName + "', 'loadStart']) // generated by script"
					}
				}
			}
			// test file content
			newFileContent += outLine + "\n"
			// read next line
			inLine = reader.readLine();
		}
		fo.append(newFileContent + "\n");
